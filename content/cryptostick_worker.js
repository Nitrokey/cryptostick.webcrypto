...
function log(aMessage) {
  ...
  var _msg = "cryptostick_worker: " + " " + aMessage + "\n";
  dump(_msg);
}
...
var WeaveCrypto = {
  ...
  initNSS : function WC_initNSS(aNSSPath) {
    ...
    this.nss_t.CK_SLOT_ID = ctypes.unsigned_long;
    ...
    this.nss_t.PK11SlotInfo = ctypes.void_t;
    this.nss_t.PK11SlotElement = ctypes.StructType(
	"PK11SlotListElement", [{ next:		ctypes.voidptr_t },
				{ prev:		ctypes.voidptr_t },
				{ slot:		this.nss_t.PK11SlotInfo.ptr },
				{ refcount:	ctypes.int }]);

    this.nss_t.PRCList = ctypes.StructType(
	"PRCList",		[{ next:	ctypes.voidptr_t },
				 { prev:	ctypes.voidptr_t }]);
    ...
    this.nss.PK11_GetInternalKeySlot = nsslib.declare("PK11_GetInternalKeySlot",
                                                      ctypes.default_abi, this.nss_t.PK11SlotInfo.ptr);
    // PK11SlotList *PK11_GetAllTokens(CK_MECHANISM_TYPE type, PRBool needRW, PRBool loadCerts, void *ctx)
    this.nss.PK11_GetAllTokens = nsslib.declare("PK11_GetAllTokens",
                                                      ctypes.default_abi, this.nss_t.PK11SlotList.ptr,
						      this.nss_t.CK_MECHANISM_TYPE,
						      this.nss_t.PRBool, this.nss_t.PRBool,
						      ctypes.voidptr_t);
    this.nss.PK11_FreeSlotList = nsslib.declare("PK11_FreeSlotList",
                                                      ctypes.default_abi, ctypes.void_t,
						      this.nss_t.PK11SlotList.ptr);
    this.nss.PK11_GetFirstSafe = nsslib.declare("PK11_GetFirstSafe",
                                                      ctypes.default_abi, this.nss_t.PK11SlotElement.ptr,
						      this.nss_t.PK11SlotList.ptr);
    this.nss.PK11_GetNextSafe = nsslib.declare("PK11_GetNextSafe",
                                                      ctypes.default_abi, this.nss_t.PK11SlotElement.ptr,
						      this.nss_t.PK11SlotList.ptr, this.nss_t.PK11SlotElement.ptr,
						      this.nss_t.PRBool);
    this.nss.PK11_GetSlotID = nsslib.declare("PK11_GetSlotID",
                                                      ctypes.default_abi, this.nss_t.CK_SLOT_ID,
						      this.nss_t.PK11SlotInfo.ptr);
    this.nss.PK11_IsHW = nsslib.declare("PK11_IsHW",
                                                      ctypes.default_abi, this.nss_t.PRBool,
						      this.nss_t.PK11SlotInfo.ptr);
    this.nss.PK11_NeedLogin = nsslib.declare("PK11_NeedLogin",
                                                      ctypes.default_abi, this.nss_t.PRBool,
						      this.nss_t.PK11SlotInfo.ptr);
  ...
  checkCryptoStick: function(slot, name, retKey) {
	...
	var ch = data.contents.toString(16);
	while (ch.length < 2)
	  ch = '0' + ch;
	astr = astr + ch;
      }
